...
__src/federation.ts__ファイルを開き、インストールしたパッケージをインポートします：

[source,typescript]
import { Temporal } from "@js-temporal/polyfill";

``Post``タイプとFedifyが提供する``PUBLIC_COLLECTION``定数もインポートします。
...
そして__src/app.tsx__ファイルを開き、先ほど定義した``<PostList>``コンポーネントをインポートします：
